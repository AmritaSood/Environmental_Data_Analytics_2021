...
# 15 
ozone.seasonal.components <- as.data.frame(Decomposed_monthly$time.series[,1:3])%>%
  mutate(extracted = GaringerOzone.monthly$MeanConcentrations,
         Date = GaringerOzone.monthly$Date)

ozone.nonseasonal.components <- ts(data = ozone.seasonal.components$extracted - ozone.seasonal.components$seasonal,
                                   start = c(2010,1),
                                   frequency = 12)

# 16
noneseasonal_trend <- Kendall::MannKendall(ozone.nonseasonal.components)
...
> Answer: The 2 sided p value is 0.0075402where we have removed the monthly seasonal component. This p value is  significant and ndicates the monotonic trend. However, when we didnt remove the seasonality component in part 14 the p value was not signigicant. We can reject the null hypothesis in this case. 
